# StockTicker · 桌面半透明摸鱼看盘神器

一个隐蔽、半透明、可置顶、能叠在任意工作窗口上的 Windows 桌面浮动行情小工具 —— 上班也能悄悄盯大盘。
自动刷新自选标的的实时行情与 K 线，支持一键隐身（`Ctrl+Alt+;`，可改）、贴边自动隐藏、无任务栏入口、透明度 10%–100% 调节、
主题切换、K 线周期切换、鼠标滚轮缩放 K 线、悬停看数值、设置自动记忆，以及一键安装/卸载的标准化安装包。
...
- **一键隐身**：默认 `Ctrl + Alt + ;`（与 QQ/微信/截图等不冲突）或点托盘图标，整个窗口秒隐藏 / 秒唤回 —— 老板来了，一秒切回「认真工作」界面；热键可在右键菜单「设置热键…」自定义。
- **贴边自动隐藏**：把窗口拖到屏幕边缘附近即吸附对齐并直接藏起整窗，不留残影色条、不留 10% 透明 —— 比手动按热键更顺手。
- **离开自动变暗**：鼠标离开窗口约 2 秒整窗自动降到 10% 透明度（无文字提示），鼠标移回即恢复。
...
- **K 线**：蜡烛图 + 悬停数值（K 线区域右下角的橙色高亮框显示该根「时间戳 + 开/高/低/收」，与 K 线坐标标签明显区分），支持滚轮缩放可见根数。
